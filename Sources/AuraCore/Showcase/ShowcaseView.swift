    ...
    private var backgroundTint: some View {
        Group {
            if settings.backgroundStyle == "gradient" {
                ...
                // Размывается маленький слой, а потом растягивается, а не
                // наоборот. Раньше обложка разворачивалась на весь экран,
                // увеличивалась ещё в 2.6 раза и только тогда размывалась
                // радиусом 130 — промежуточные буферы под такое уходят
                // в гигабайты: пик у процесса доходил до 4.5 ГБ, и после
                // ночи с витриной приложение держало 286 МБ вместо 80.
                //
                // Картинка от этого не меняется: при радиусе в сотню точек
                // от исходных деталей всё равно ничего не остаётся. 10 × 13
                // даёт те же 130 точек размытия в итоговом масштабе.
