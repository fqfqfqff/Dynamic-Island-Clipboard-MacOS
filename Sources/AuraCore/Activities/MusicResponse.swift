...
struct MusicResponse: Equatable {
    ...
    /// Плеер отвечает пятью или шестью строками. Пустой заголовок означает,
    /// что играть нечего, — такой ответ не считается результатом.
    static func parse(_ text: String, durationDivisor: Double) -> MusicResponse? {
        ...
        let artworkURL = lines.count >= 6 && !lines[5].isEmpty ? lines[5] : nil
