        ...
        VStack(alignment: .leading, spacing: 22) {
            ...
            standard {
                group(t("ui.5266f583", "Текст песни")) {
                    ...
                    // Подчинённый переключатель показывается только когда
                    // есть чему подчиняться: без текстов строка в острове
                    // ничего не значит.
                    if settings.showLyrics {
                        Toggle(t("ui.5c02fa71", "Строка песни в раскрытом острове"),
                               isOn: $settings.showLyricsInPanel)
                            .padding(.leading, 18)
                        hint("Строка идёт под названием и меняется по ходу песни. Витрина показывает текст целиком, но она отдельный полноэкранный режим, а строка нужна там же, где играет музыка.")
                    }
                }
            }
    ...
    private var notifications: some View {
        VStack(alignment: .leading, spacing: 22) {
            ...
            group(t("ui.a1c73f52", "Значки приложений")) {
                ...
                if settings.readIconsFromBanner {
                    Toggle(t("ui.4a19c7e0", "Срезать уголок со значком телефона"),
                           isOn: $settings.trimPhoneBadge)
                        .padding(.leading, 18)
                    hint("Уведомление с айфона macOS помечает маленьким телефоном в углу иконки. В снимок он попадает вместе с ней и выглядит грязью. Срезается правый нижний угол — иконки редко несут что-то важное в самом углу. Если у какого-то приложения срезалось лишнее, выключите.")
                }
